...
function cacheToolDef(source: BotReadToolDefinition): ToolDef {
  ...
  properties.source_message_id = {
    ...
    description:
      "Обязательный служебный ID сообщения-триггера текущего хода, который подставляет trusted bridge (OpenClaw parilka-chat скрывает его и подставляет inbound Telegram message id). Инструмент никогда не возвращает это сообщение или более новые.",
  };
  const required = [
