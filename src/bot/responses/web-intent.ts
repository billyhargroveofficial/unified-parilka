...
const EXPLICIT_RESEARCH = /(?<![\p{L}\p{N}_])(?:deep[\s_-]?(?:dive|research)|дип[\s_-]?(?:дайв|ресерч)|дипдайв|(?:глубок\p{L}*|детальн\p{L}*|тщательн\p{L}*|нереальн\p{L}*)\s+(?:(?:deep|дип)[\s_-]?)?(?:research|ресерч|исследован\p{L}*)|(?:проведи|провести|сделай|сделать|нужен|нужно)\s+(?:\p{L}+\s+){0,3}(?:research|ресерч|исследован\p{L}*))(?![\p{L}\p{N}_])/iu;
const NEGATED_RESEARCH = /(?<![\p{L}\p{N}_])(?:без|не\s+(?:делай|делать|проводи|проводить|нужен|нужно|надо))\s+(?:(?:делать|проводить)\s+)?(?:(?:deep|дип|глубок\p{L}*|детальн\p{L}*|тщательн\p{L}*)[\s_-]+)?(?:dive|дайв|deepdive|дипдайв|research|ресерч\p{L}*|исследован\p{L}*)(?![\p{L}\p{N}_])/iu;
