...
import type { HermesMemoryLock } from "./types.js";

export class HermesLockHeldError extends Error {
  readonly name = "HermesLockHeldError";
  readonly code = "hermes_lock_held";
}

export class HermesLockTimeoutError extends Error {
  readonly name = "HermesLockTimeoutError";
  readonly code = "hermes_lock_timeout";
}

const FLOCK_BIN = "/usr/bin/flock";

/**
 * Acquire an fcntl flock on <profileHome>/memories/MEMORY.md.lock,
 * interoperating with Hermes's native blocking flock.
 *
...
export function acquireHermesMemoryLock(
  ...
  try {
    ...
    if (lockStat.isSymbolicLink()) {
      return Promise.reject(
        new HermesLockHeldError(
          "Hermes memory lock file must not be a symlink.",
        ),
      );
    }
    if (!lockStat.isFile()) {
      return Promise.reject(
        new HermesLockHeldError(
          "Hermes memory lock file must be a regular file.",
        ),
      );
    }
  } catch (error) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      child.kill("SIGKILL");
      reject(
        new HermesLockTimeoutError(
          `Could not acquire Hermes memory lock within ${timeoutMs}ms.`,
        ),
      );
    }, timeoutMs + 5_000);
    ...
    child.on("exit", (code, signal) => {
      ...
      if (signal) {
        reject(
          new HermesLockTimeoutError(
            `Hermes lock child killed by signal ${signal}.`,
          ),
        );
      } else if (code === 1) {
        ...
        reject(
          new HermesLockHeldError(
            "Hermes memory lock is held by another process.",
          ),
        );
      }
    });
